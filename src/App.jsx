...
export default function App() {
  ...
  const handleDownload = async (downloadUrl, filename) => {
    ...
    try {
      ...
      link.href = blobUrl;
      link.download = `${filename.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.mp4`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(blobUrl);
    } catch (err) {
      // Fallback: Jika gagal fetch karena CORS, buka di tab baru
    ...
    <div className="min-h-screen p-6 flex flex-col items-center justify-center font-sans overflow-x-hidden relative bg-[#050505]">
      ...
      <header className="mb-10 text-center flex flex-col items-center w-full max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-8">
           ...
           <h2 className="text-[10px] sm:text-xs font-medium tracking-[0.5em] text-cyan-400/80 uppercase whitespace-nowrap">NO WATERMARK ENGINE</h2>
           <div className="h-[1px] w-8 bg-gradient-to-l from-transparent to-cyan-500/50"></div>
        </div>
      </header>

      {/* PLATFORM GRID */}
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="w-full max-w-4xl grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-10">
        {platforms.map((p) => (
          <motion.button
            key={p.id}
            ...
            whileTap={{ scale: 0.98 }}
            className={`relative flex flex-row items-center justify-start gap-3 px-3 sm:px-4 py-3 sm:py-4 rounded-xl border border-white/5 bg-[#0a0a0c] hover:bg-[#121214] transition-all duration-300 group overflow-hidden ${selected === p.id ? 'ring-1 ring-offset-1 ring-offset-black' : ''}`}
          ...
          >
            ...
            <div className="transition-transform duration-300 group-hover:scale-110 shrink-0" style={{ color: p.color }}>{React.cloneElement(p.icon, { size: 18 })}</div>
            <span className="text-[10px] sm:text-xs font-bold tracking-wider text-gray-400 uppercase group-hover:text-white transition-colors truncate">{p.name}</span>
            {selected === p.id && <motion.div layoutId="active-dot" className="absolute right-3 w-1.5 h-1.5 rounded-full" style={{ backgroundColor: p.color, boxShadow: `0 0 10px ${p.color}` }} />}
          </motion.button>
        ))}
      </motion.div>
      ...
      <AnimatePresence mode="wait">
        {result && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="w-full max-w-3xl mb-8">
            <div className="bg-[#0a0a0c] border border-white/10 rounded-2xl overflow-hidden">
              <div className="bg-white/5 px-6 py-3 border-b border-white/5 flex justify-between items-center">
                <div className="flex items-center gap-2 text-green-400 text-xs font-bold tracking-wider"><CheckCircle size={14} /> SUCCESSFULLY EXTRACTED</div>
                <div className="text-[10px] text-gray-500 font-mono">Just now</div>
              </div>
              <div className="p-6 flex flex-col md:flex-row gap-6">
                <div className="w-full md:w-1/3 aspect-video bg-black rounded-xl border border-white/5 relative overflow-hidden group shadow-lg">
                  ...
                  <div className="absolute bottom-2 left-2 text-[10px] text-white bg-black/60 backdrop-blur-md px-2 py-0.5 rounded border border-white/10">HD Quality</div>
                </div>
                <div className="flex-1 flex flex-col justify-between gap-4">
                  <div>
                    <h3 className="text-white font-bold text-lg mb-1 line-clamp-2 leading-snug">{result.title}</h3>
                    <p className="text-gray-400 text-xs flex items-center gap-1"><Activity size={10}/> By {result.author || 'Creator'}</p>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    {primaryLink ? (
                      <button 
                        onClick={() => handleDownload(primaryLink, result.title)}
                    ...
                    {showAudioButton && (
                      getDownloadLink('audio') ? (
                        <button 
                          onClick={() => handleDownload(getDownloadLink('audio'), `audio_${result.title}`)}
      ...
      {/* HISTORY & STATS (Tetap Sama) */}
      {/* ... (bagian History, Stats, Donate, Footer tetap sama seperti aslinya) */}
      {/* (Gue persingkat biar nggak kena limit karakter, tapi intinya bagian bawah tetap lo pertahankan) */}
      
      {/* HISTORY */}
      {history.length > 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-3xl mt-4">
          <div className="flex items-center justify-between mb-3 px-2">
            <div className="flex items-center gap-2 text-gray-500 text-[10px] font-bold tracking-widest uppercase"><Clock size={12} /> Recent History</div>
            <button onClick={clearHistory} className="text-[10px] text-red-500/70 hover:text-red-500 flex items-center gap-1 transition-colors"><Trash2 size={10} /> CLEAR ALL</button>
          </div>
          <div className="space-y-2">
            {history.map((item, idx) => (
              <motion.div key={idx} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="bg-[#0a0a0c] border border-white/5 rounded-lg p-3 flex items-center justify-between hover:border-white/10 transition-colors group">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-white/5 rounded-md text-gray-400 group-hover:text-white transition-colors"><Activity size={14} /></div>
                ...
                <button className="text-gray-500 hover:text-cyan-400 text-[10px] border border-transparent hover:border-cyan-400/30 px-3 py-1.5 rounded transition-all bg-white/5">OPEN</button>
              </motion.div>
            ))}
          </div>
        </motion.div>
      ...
      {/* REALTIME STATS */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-3xl mt-12 grid grid-cols-2 gap-4">
        <div className="bg-[#0a0a0c] border border-white/5 rounded-2xl p-5 flex items-center justify-between group hover:border-cyan-500/20 transition-colors">
           ...
           <div className="p-3 bg-cyan-500/10 rounded-xl text-cyan-400 group-hover:scale-110 transition-transform"><Users size={20} /></div>
        </div>
        <div className="bg-[#0a0a0c] border border-white/5 rounded-2xl p-5 flex items-center justify-between group hover:border-green-500/20 transition-colors">
           ...
           <div className="p-3 bg-green-500/10 rounded-xl text-green-400 group-hover:scale-110 transition-transform"><BarChart3 size={20} /></div>
        </div>
      </motion.div>

      {/* DONATE & FOOTER */}
      <motion.a href="https://saweria.co/nebulaajajan" target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex items-center gap-2 bg-gradient-to-r from-yellow-500 to-orange-500 text-black font-bold text-xs px-5 py-3 rounded-full shadow-[0_0_20px_rgba(255,165,0,0.4)] mx-auto mt-10 md:fixed md:bottom-6 md:right-6 md:z-50 md:m-0">
        <Heart size={16} className="fill-black animate-pulse" /> DONATE
      </motion.a>

      <footer className="mt-16 text-center opacity-30 hover:opacity-100 transition-opacity pb-8">
        <p className="text-[10px] text-white font-mono tracking-[0.2em] mb-2">© 2026 Lyxkall Downloader. All rights reserved.</p>
        <div className="flex items-center justify-center gap-2 text-[9px] text-white"><span>Powered by Lyxkall</span></div>
      </footer>
    </div>
  );
}